...
Finally, let's plug in an `input` tag, which has a couple of attributes. One attribute tells us what type of input it is = type equals button - and the other tells us the value - what does it say on the button? `Click me!` Now, notice that the `input` tag doesn't have a closing tag. 
